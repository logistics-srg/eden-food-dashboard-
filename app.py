...
# ══════════════════════════════════════════════════════════════════════════════
#  DESIGN SYSTEM — UUPM Pro Max + 21st.dev Magic
#  Tokens: couleurs, spacing, typographie, shadows, animations
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
.kpi-card .kpi-sub{font-size:11px;color:var(--text-muted);font-weight:400}
...
.sec-title{font-size:14px;font-weight:700;color:var(--text-primary);letter-spacing:-0.2px}
...
/* ══ PILLS ════════════════════════════════════════════════════════════════ */
.pill{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;border-radius:var(--radius-full);font-size:11px;font-weight:700;letter-spacing:0.2px}
...
.pill-indigo{background:var(--primary-light);color:#3730A3}
...
.hero-badge{
  ...
  color:#fff;padding:7px 18px;border-radius:var(--radius-full);
  font-size:12px;font-weight:600;letter-spacing:0.2px;
}

/* ══ ALERTS ═══════════════════════════════════════════════════════════════ */
.alert{padding:11px 15px;border-radius:var(--radius-sm);font-size:12px;font-weight:600;margin:6px 0;display:flex;align-items:center;gap:8px}
...
.prog-fill{height:100%;border-radius:var(--radius-full);transition:width 0.6s ease}
...
.doc-chip{
  ...
  padding:5px 10px;font-size:11px;color:var(--text-secondary);font-weight:500;
  transition:var(--transition);margin:3px;
}
.doc-chip:hover{border-color:var(--primary);color:var(--primary);box-shadow:var(--shadow-sm)}

/* ══ FORM ═════════════════════════════════════════════════════════════════ */
div[data-testid="stForm"]{
  background:var(--surface)!important;border-radius:var(--radius-lg)!important;
  padding:26px!important;border:1px solid var(--border)!important;
...
def apply_chart_style(fig, bgcolor="#fff"):
    fig.update_layout(
        ...
        font=dict(family="Inter, Arial", size=12, color="#111827"),
        margin=dict(t=20,b=20,l=20,r=20),
        legend=dict(font=dict(color="#111827", size=12)),
    )
    fig.update_xaxes(tickfont=dict(color="#6B7280"), title_font=dict(color="#6B7280"),
                     showgrid=False, linecolor="#E5E7EB")
    fig.update_yaxes(tickfont=dict(color="#6B7280"), title_font=dict(color="#6B7280"),
                     gridcolor="#F3F4F6", linecolor="transparent")
    return fig
...
def login_page():
    ...
    logo_html = (f'<img src="{logo_src}" style="height:52px;margin-bottom:20px;display:block;margin:0 auto 20px">'
    ...
    .login-divider{{height:1px;background:linear-gradient(90deg,transparent,#E5E7EB,transparent);margin:20px 0}}
    </style>
    ...
    <div style="position:relative;z-index:1;display:flex;align-items:center;justify-content:center;min-height:82vh">
      <div class="login-card">
        ...
        <p style="font-size:13px;color:#9CA3AF;margin:0 0 4px;font-weight:400">Logistics Platform</p>
        <div style="display:inline-flex;align-items:center;gap:6px;background:#D1FAE5;color:#065F46;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:700;margin-bottom:20px">🟢 Système opérationnel</div>
...
with st.sidebar:
    ...
    for pid, icon, label in NAV:
        active = st.session_state.page == pid
        if active:
            st.markdown(f'<div style="background:#EEF2FF;border-radius:8px;padding:10px 14px;margin-bottom:1px;'
            ...
            st.button(f"_active_{pid}", key=f"nav_active_{pid}", use_container_width=True,
    ...
    st.markdown('<div style="position:absolute;bottom:0;left:0;right:0;padding:14px 18px;border-top:1px solid #E5E7EB;background:#fff">', unsafe_allow_html=True)
    st.markdown(f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'
                f'<div style="width:32px;height:32px;border-radius:50%;background:#EEF2FF;display:flex;align-items:center;justify-content:center;font-size:14px">👤</div>'
                f'<div><div style="font-size:12px;font-weight:700;color:#111827">{st.session_state.username.upper()}</div>'
                f'<div style="font-size:10px;color:#9CA3AF">{st.session_state.role}</div></div></div>',
                unsafe_allow_html=True)
    if st.button("🚪  Déconnexion", use_container_width=True, key="logout"):
...
if page == "dashboard":
    ...
    if hero_src:
        ...
        <div class="hero-wrap">
          ...
          <div class="hero-overlay">
            <div class="hero-text">
              ...
              <p>Suivi en temps réel de vos expéditions — Colombie & Costa Rica</p>
        ...
        st.markdown(f"""
        <div style="background:linear-gradient(110deg,#0F172A,#4361EE);padding:52px 52px;position:relative;overflow:hidden">
          ...
          <p style="font-size:14px;color:rgba(255,255,255,0.72);max-width:440px;margin:0 0 22px">Colombie & Costa Rica — Suivi logistique en temps réel</p>
          <div class="hero-badge">🍌 &nbsp;{current_week_str} &nbsp;·&nbsp; {len(commandes)} expéditions actives</div>
        </div>""", unsafe_allow_html=True)
    ...
    <div class="kpi-grid">
      <div class="kpi-card" style="--card-color:#4361EE;--card-bg:#EEF2FF">
        ...
        <div class="kpi-lbl">Expéditions</div>
        <div class="kpi-val" style="color:#4361EE">{len(commandes)}</div>
        <div class="kpi-sub">enregistrées</div>
      </div>
      <div class="kpi-card" style="--card-color:#F59E0B;--card-bg:#FEF3C7">
        <div class="icon-wrap">⏳</div>
        <div class="kpi-lbl">À générer</div>
        <div class="kpi-val" style="color:#D97706">{len(todo)}</div>
      ...
      <div class="kpi-card" style="--card-color:#10B981;--card-bg:#D1FAE5">
        ...
        <div class="kpi-val" style="color:#059669">{len(done)}</div>
        <div class="kpi-sub">générées</div>
      </div>
      <div class="kpi-card" style="--card-color:#8B5CF6;--card-bg:#EDE9FE">
        <div class="icon-wrap">📦</div>
        <div class="kpi-lbl">CNT planifiés</div>
        <div class="kpi-val" style="color:#7C3AED">{int(todo["nb_cnt"].sum())}</div>
        <div class="kpi-sub">conteneurs</div>
      </div>
      <div class="kpi-card" style="--card-color:#EF4444;--card-bg:#FEE2E2">
        <div class="icon-wrap">🔴</div>
        <div class="kpi-lbl">Alertes</div>
        <div class="kpi-val" style="color:#DC2626">{len(alerte)}</div>
        <div class="kpi-sub">licences critiques</div>
      </div>
      <div class="kpi-card" style="--card-color:#3B82F6;--card-bg:#DBEAFE">
        <div class="icon-wrap">📁</div>
        <div class="kpi-lbl">Documents</div>
        <div class="kpi-val" style="color:#2563EB">{total_docs}</div>
        <div class="kpi-sub">uploadés</div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    st.markdown('<div class="sec-hdr"><span class="sec-title">Volume hebdomadaire</span><span class="sec-sub">conteneurs par semaine</span></div>', unsafe_allow_html=True)
    if not commandes.empty:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="topbar">
      <div style="display:flex;align-items:center;gap:14px">
        <div>
          <div class="topbar-title">Semaine en cours</div>
          <div style="font-size:12px;color:#9CA3AF;margin-top:2px">Semaine {current_week_num} · {datetime.now().year}</div>
        </div>
        <div class="topbar-badge">{current_week_str}</div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    if commandes_semaine.empty:
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:26px">
          <div class="kpi-card" style="--card-color:#4361EE;--card-bg:#EEF2FF">
            <div class="kpi-lbl">Total</div><div class="kpi-val" style="color:#4361EE">{len(commandes_semaine)}</div>
          </div>
          <div class="kpi-card" style="--card-color:#F59E0B;--card-bg:#FEF3C7">
            <div class="kpi-lbl">⏳ À générer</div><div class="kpi-val" style="color:#D97706">{len(todo_s)}</div>
          </div>
          <div class="kpi-card" style="--card-color:#10B981;--card-bg:#D1FAE5">
            <div class="kpi-lbl">✅ Confirmées</div><div class="kpi-val" style="color:#059669">{len(done_s)}</div>
          </div>
          <div class="kpi-card" style="--card-color:#8B5CF6;--card-bg:#EDE9FE">
            <div class="kpi-lbl">CNT total</div><div class="kpi-val" style="color:#7C3AED">{int(commandes_semaine["nb_cnt"].sum())}</div>
          </div>
        </div>""", unsafe_allow_html=True)

        st.markdown('<div class="sec-hdr"><span class="sec-title">Commandes de la semaine</span></div>', unsafe_allow_html=True)

        for _, row in commandes_semaine.iterrows():
            is_done = "GÉNÉRÉ" in str(row["statut"])
            pill_cls = "pill-green" if is_done else "pill-orange"
            n_docs   = len(list_docs(row["booking"]))
            doc_pill = f'<span class="pill pill-blue">📁 {n_docs}</span>' if n_docs else ''
            ...
            <div class="cmd-row">
              ...
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px">POL</div>
                <div style="font-weight:700;color:#111827;font-size:12px">{row['pol']}</div></div>
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px">CNT</div>
                <div style="font-weight:900;color:#4361EE;font-size:22px;letter-spacing:-1px">{row['nb_cnt']}</div></div>
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px">Départ</div>
                <div style="font-weight:600;color:#374151;font-size:12px">{row['depart']}</div></div>
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px">ETA</div>
                <div style="font-weight:600;color:#374151;font-size:12px">{row['eta']}</div></div>
              <div style="display:flex;gap:6px;align-items:center">
                {doc_pill}
                <span class="pill {pill_cls}">{row['statut']}</span>
              </div>
            </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with fc4:
        f_sem = st.text_input("Semaine", placeholder="S-18", label_visibility="collapsed")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="font-size:12px;color:#6B7280;margin-bottom:14px;font-weight:500">'
    ...
    for _, row in df_filt.iterrows():
        ...
        n_docs   = len(list_docs(row["booking"]))
        doc_pill = f'<span class="pill pill-blue">📁 {n_docs}</span>' if n_docs else \
                   '<span class="pill" style="background:#F3F4F6;color:#9CA3AF">📁 0</span>'

        st.markdown(f"""
        <div class="cmd-row">
          <div style="min-width:200px">
            <div style="font-size:13px;font-weight:700;color:#111827">{row['client']}</div>
            <div style="font-size:11px;color:#9CA3AF;margin-top:2px">{row['semaine']} · {row['booking']}</div>
          </div>
          <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px;text-transform:uppercase">Navire</div>
            <div style="font-weight:500;color:#374151;font-size:11px;max-width:130px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{row['navire']}</div></div>
          ...
          <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px;text-transform:uppercase">CNT</div>
            <div style="font-weight:900;color:#4361EE;font-size:20px;letter-spacing:-1px">{row['nb_cnt']}</div></div>
          <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px;text-transform:uppercase">Départ</div>
            <div style="font-weight:500;color:#374151;font-size:11px">{row['depart']}</div></div>
          <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px;text-transform:uppercase">ETA</div>
            <div style="font-weight:500;color:#374151;font-size:11px">{row['eta']}</div></div>
          <div style="display:flex;gap:6px;align-items:center">{doc_pill}<span class="pill {pill_cls}">{row['statut']}</span></div>
        </div>""", unsafe_allow_html=True)

        # Gestion documents inline
        is_open = st.session_state.expanded_cmd == row['booking']
        col_btn, _ = st.columns([1,5])
        with col_btn:
            lbl = "🔼 Fermer" if is_open else "📁 Documents"
            if st.button(lbl, key=f"docbtn_{row['booking']}", use_container_width=True):
            ...
            with st.container():
                ...
                <div class="doc-zone">
                  <div style="font-size:12px;font-weight:700;color:#111827;margin-bottom:12px">
                    📁 Documents — <span style="color:#4361EE">{row['booking']}</span>
                  </div>
                </div>""", unsafe_allow_html=True)
                ...
                if existing:
                    ...
                    st.caption("Aucun document — uploadez-en un ci-dessous.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:26px">
      ...
      <div class="kpi-card" style="--card-color:#10B981;--card-bg:#D1FAE5">
        <div class="icon-wrap">✅</div><div class="kpi-lbl">Documentées</div>
        <div class="kpi-val" style="color:#059669">{cmds_avec}</div>
        <div class="kpi-sub">commandes avec docs</div>
      </div>
      <div class="kpi-card" style="--card-color:#F59E0B;--card-bg:#FEF3C7">
        <div class="icon-wrap">⚠️</div><div class="kpi-lbl">Sans documents</div>
        <div class="kpi-val" style="color:#D97706">{cmds_sans}</div>
    ...
    for _, row in commandes.iterrows():
        if search and search.lower() not in str(row["booking"]).lower() \
                  and search.lower() not in str(row["client"]).lower():
            continue
        ...
        <div class="cmd-row" style="border-left:3px solid {clr}">
          <div style="min-width:220px">
            <div style="font-size:13px;font-weight:700;color:#111827">{row['client']}</div>
            <div style="font-size:11px;color:#9CA3AF">{row['booking']} · {row['semaine']}</div>
          </div>
          <div><span style="color:{clr};font-weight:700;font-size:12px">{lbl}</span></div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for _, row in clients.iterrows():
        pct_r = max(0,min(100, row["solde_reel"]/row["poids_total"]*100)) if row["poids_total"] > 0 else 0
        pct_p = max(0,min(100, row["solde_prev"]/row["poids_total"]*100)) if row["poids_total"] > 0 else 0
        prog_r_clr = "#10B981" if pct_r > 30 else ("#F59E0B" if pct_r > 10 else "#EF4444")
        ...
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;margin-bottom:16px">
            <div>
              <div style="font-size:16px;font-weight:800;color:#111827;letter-spacing:-0.3px">{row['nom']}</div>
              <div style="font-size:12px;color:#9CA3AF;margin-top:3px">🔑 {row['licence']} · {row['pays']}</div>
            </div>
            {badge}
          </div>
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px">
            <div style="background:#F9FAFB;border-radius:10px;padding:14px">
              <div style="font-size:9px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;margin-bottom:4px">Poids total</div>
            ...
            <div style="background:#EEF2FF;border-radius:10px;padding:14px">
              <div style="font-size:9px;color:#4361EE;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;margin-bottom:4px">Solde réel</div>
            ...
            <div style="background:#FEF3C7;border-radius:10px;padding:14px">
              <div style="font-size:9px;color:#D97706;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;margin-bottom:4px">Solde prévi.</div>
          ...
          <div style="margin-bottom:8px">
            <div style="display:flex;justify-content:space-between;margin-bottom:4px">
              <span style="font-size:11px;color:#6B7280;font-weight:500">Solde réel</span>
              <span style="font-size:11px;font-weight:700;color:{prog_r_clr}">{pct_r:.0f}% disponible</span>
            </div>
            <div class="prog-track"><div class="prog-fill" style="background:{prog_r_clr};width:{pct_r}%"></div></div>
          </div>
          <div>
            <div style="display:flex;justify-content:space-between;margin-bottom:4px">
              <span style="font-size:11px;color:#6B7280;font-weight:500">Solde prévisionnel</span>
              <span style="font-size:11px;font-weight:700;color:#D97706">{pct_p:.0f}% disponible</span>
            </div>
            <div class="prog-track"><div class="prog-fill" style="background:#F59E0B;width:{pct_p}%"></div></div>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        if pdf_exists:
            ...
            st.caption(f"⚠️ PDF non trouvé → `{licence_pdf_path(row['licence'])}`")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if client_sel:
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:26px">
          <div class="kpi-card" style="--card-color:#4361EE;--card-bg:#EEF2FF">
            <div class="kpi-lbl">Total CNT</div><div class="kpi-val" style="color:#4361EE">{int(df_c["nb_cnt"].sum())}</div>
          </div>
          <div class="kpi-card" style="--card-color:#8B5CF6;--card-bg:#EDE9FE">
            <div class="kpi-lbl">Total kgs</div><div class="kpi-val" style="color:#7C3AED;font-size:1.5rem">{df_c["total_kgs"].sum():,.0f}</div>
          </div>
          <div class="kpi-card" style="--card-color:#F59E0B;--card-bg:#FEF3C7">
            <div class="kpi-lbl">⏳ En cours</div><div class="kpi-val" style="color:#D97706">{len(df_c[df_c["statut"].str.contains("À GÉNÉRER",na=False)])}</div>
          </div>
          <div class="kpi-card" style="--card-color:#10B981;--card-bg:#D1FAE5">
            <div class="kpi-lbl">✅ Confirmées</div><div class="kpi-val" style="color:#059669">{len(df_c[df_c["statut"].str.contains("GÉNÉRÉ",na=False)])}</div>
          </div>
        </div>""", unsafe_allow_html=True)

        st.markdown('<div class="sec-hdr"><span class="sec-title">Expéditions</span></div>', unsafe_allow_html=True)
        ...
        for _, row in df_c.sort_values("semaine",ascending=False).iterrows():
            ...
            <div class="cmd-row">
              ...
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px">POL</div>
                <div style="font-weight:700;color:#111827">{row['pol']}</div></div>
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px">CNT</div>
                <div style="font-weight:900;color:#4361EE;font-size:20px">{row['nb_cnt']}</div></div>
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px">Départ</div>
                <div style="font-weight:500;color:#374151;font-size:12px">{row['depart']}</div></div>
              <div style="text-align:center"><div style="font-size:9px;color:#9CA3AF;margin-bottom:2px">ETA</div>
                <div style="font-weight:500;color:#374151;font-size:12px">{row['eta']}</div></div>
              <span class="pill pill-blue">📁 {n_docs}</span>
              <span class="pill {pill_cls}">{row['statut']}</span>
            </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("form_cmd", clear_on_submit=True):
        ...
        with f1:
            ...
            licence_sel = st.selectbox("Licence *", lic_dispo)
        with f2:
        ...
        if   solde_apres < 0:       st.markdown('<div class="alert alert-red">⚠️ Solde insuffisant — commande bloquée</div>', unsafe_allow_html=True)
        elif solde_apres < 19591.2: st.markdown('<div class="alert alert-warn">🟠 Solde critique après cette commande</div>', unsafe_allow_html=True)
        else:                        st.markdown('<div class="alert alert-ok">✅ Solde suffisant</div>', unsafe_allow_html=True)

        if st.form_submit_button("✅ Enregistrer la commande", use_container_width=True,
            ...
            st.session_state.new_commandes.append({
                ...
                "licence":licence_sel,"navire":navire,"voyage":voyage,"pol":pol_sel,
